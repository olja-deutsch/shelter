...
function toggleLogoClass() {
    document.querySelector('.logo').classList.toggle('logo_light');

    if (!document.body.classList.contains('show-menu')) {
        document.querySelector('.logo').classList.remove('logo_light');
    }
}

document.querySelector('.hamburger-button').addEventListener('click', toggleLogoClass);
document.querySelector('.blackout').addEventListener('click', toggleLogoClass);
...
request.onload = () => {
    ...
    catalogItems.addEventListener('click', showPopup);

    function showPopup(e) {
        if (e.target.closest('.catalog__content')) {
            document.body.classList.add('show-popup');
            document.querySelector('.blackout').classList.add('active');

            const currentPetName = e.target.closest('.catalog__content').getAttribute('data-name');
